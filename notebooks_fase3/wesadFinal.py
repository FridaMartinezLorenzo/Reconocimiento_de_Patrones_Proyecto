...
# ===========================================================================
# GRÁFICAS SEPARADAS — una figura .jpg por combinación modelo × tipo
# ===========================================================================
...
# ===========================================================================
print("\nGenerando matrices de correlación separadas...")

# ── F1) Correlación — Top-20 features más discriminativas (ranking t-test) ──
top20_features = diff_df.head(20).index.tolist()
...
# ── F2) Correlación — Top-5 features seleccionadas por Fisher ───────────────
corr_top5 = X[selected_features].corr()

fig, ax = plt.subplots(figsize=(7, 6))
sns.heatmap(corr_top5, cmap='RdBu_r', center=0,
            annot=True, fmt='.3f', square=True, ax=ax,
            linewidths=1.0, cbar_kws={"shrink": 0.8},
            annot_kws={"size": 10})
...
# ── F3) Correlación por grupo de sensor ─────────────────────────────────────
# Identificar prefijos de sensor presentes en X_scaled
sensor_groups = {
    'ECG (chest)'   : [c for c in X_scaled.columns if c.startswith('c_ECG')],
    'EDA (chest)'   : [c for c in X_scaled.columns if c.startswith('c_EDA')],
    'EMG (chest)'   : [c for c in X_scaled.columns if c.startswith('c_EMG')],
    'RESP (chest)'  : [c for c in X_scaled.columns if c.startswith('c_RESP')],
    'TEMP (chest)'  : [c for c in X_scaled.columns if c.startswith('c_TEMP')],
    'ACC (chest)'   : [c for c in X_scaled.columns if c.startswith('c_ACC')],
    'BVP (wrist)'   : [c for c in X_scaled.columns if c.startswith('w_BVP')],
    'EDA (wrist)'   : [c for c in X_scaled.columns if c.startswith('w_EDA')],
    'TEMP (wrist)'  : [c for c in X_scaled.columns if c.startswith('w_TEMP')],
    'ACC (wrist)'   : [c for c in X_scaled.columns if c.startswith('w_ACC')],
}

for sensor_name, sensor_cols in sensor_groups.items():
    if len(sensor_cols) < 2:
        continue   # no hay suficientes features para una matriz útil

    corr_sensor = X_scaled[sensor_cols].corr()
    mask_sensor = np.triu(np.ones_like(corr_sensor, dtype=bool))
    n = len(sensor_cols)
    fig_size = max(5, n * 0.7)

    fig, ax = plt.subplots(figsize=(fig_size, fig_size * 0.9))
    sns.heatmap(corr_sensor, mask=mask_sensor, cmap='RdBu_r', center=0,
                annot=True, fmt='.2f', square=True, ax=ax,
                linewidths=0.5, cbar_kws={"shrink": 0.8},
                annot_kws={"size": max(6, 10 - n // 3)})
    ax.set_title(f'Matriz de Correlaciones — {sensor_name}',
                 fontsize=12, fontweight='bold')
    plt.tight_layout()
    slug = sensor_name.replace(' ', '_').replace('(', '').replace(')', '')
    save_jpg(fig, f'fig3_corr_sensor_{slug}.jpg')

# ── F4) Correlación — todas las features preprocesadas (mapa de calor compacto) ──
corr_all = X_scaled.corr()
n_all_feats = len(X_scaled.columns)
fig_w = min(28, max(14, n_all_feats * 0.25))
fig_h = min(24, max(12, n_all_feats * 0.22))
...
print("    fig3_corr_top20_discriminativas.jpg   (top-20 features por t-test)")
print("    fig3_corr_top5_fisher.jpg             (top-5 features Fisher)")
print("    fig3_corr_todas_features.jpg          (todas las features preprocesadas)")
print("    fig3_corr_sensor_<nombre>.jpg         (una por cada sensor/grupo)")
