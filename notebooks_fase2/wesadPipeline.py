...
5. Selección de 5 mejores características con Selección Escalar Hacia Adelante
...
def fisher_score(X_data, y_data):
    """
    Calcula el Factor de Fisher (Fisher Discriminant Ratio) para cada característica.
    
    El Factor de Fisher mide la capacidad discriminativa de una característica
    como la razón entre la varianza entre clases y la varianza dentro de clases:
    
    F(f) = (μ₁ - μ₂)² / (σ₁² + σ₂²)
    
    Donde:
    - μ₁, μ₂: medias de la característica para cada clase
    - σ₁², σ₂²: varianzas de la característica para cada clase
    
    Un valor mayor indica mejor capacidad discriminativa.
    """
    ...
    for i in range(n_features):
        feature_values = X_data.iloc[:, i] if hasattr(X_data, 'iloc') else X_data[:, i]
        ...
        for c in classes:
            ...
            means.append(np.mean(class_values))
            variances.append(np.var(class_values))
        ...
        if len(classes) == 2:
            ...
            # Generalización para multiclase
            overall_mean = np.mean(feature_values)
            numerator = sum(n * (m - overall_mean) ** 2 for n, m in zip(n_samples, means))
            denominator = sum(n * v for n, v in zip(n_samples, variances))
...
# ============================================================================
# ACTIVIDAD 5: SELECCIÓN ESCALAR HACIA ADELANTE (FORWARD SELECTION)
# ============================================================================
print("\n\n" + "=" * 80)
print("ACTIVIDAD 5: SELECCIÓN ESCALAR HACIA ADELANTE (5 MEJORES CARACTERÍSTICAS)")
print("=" * 80)
...
def sequential_forward_selection(X_data, y_data, n_features_to_select=5, 
    ...
    2. En cada iteración, evaluar CADA característica candidata añadiéndola
       al conjunto seleccionado y midiendo el accuracy con validación cruzada.
    3. Seleccionar la característica que maximiza el accuracy.
    4. Repetir hasta alcanzar el número deseado de características.
    
    Parámetros:
    - X_data: DataFrame con las características
    - y_data: Series con las etiquetas
    - n_features_to_select: número de características a seleccionar
    - classifier: clasificador a usar (default: KNN k=5)
    - cv_folds: número de folds para validación cruzada
    
    Retorna:
    - selected_features: lista de características seleccionadas
    - history: historial de cada paso
    """
    if classifier is None:
        classifier = KNeighborsClassifier(n_neighbors=5)
    
    available_features = list(X_data.columns)
    selected_features = []
    history = []
    
    print(f"\nIniciando SFS con {len(available_features)} características candidatas...")
    print(f"Objetivo: seleccionar las {n_features_to_select} mejores")
    print(f"Clasificador: {classifier.__class__.__name__}")
    print(f"Validación cruzada: {cv_folds} folds")
    
    for step in range(n_features_to_select):
        print(f"\n--- Paso {step + 1}/{n_features_to_select} ---")
        best_score = -1
        best_feature = None
        step_results = []
        
        for feat in available_features:
            # Conjunto candidato = seleccionadas + nueva
            candidate_features = selected_features + [feat]
            X_candidate = X_data[candidate_features]
            
            # Validación cruzada estratificada
            cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
            scores = cross_val_score(classifier, X_candidate, y_data, 
        ...
        history.append({
            ...
            'all_evaluated': sorted(step_results, key=lambda x: -x['accuracy'])
        })
        
        print(f"  Característica seleccionada: {best_feature}")
        print(f"  Accuracy con {step+1} feature(s): {best_score:.4f}")
        
        # Mostrar top 5 candidatas del paso
        top5_candidates = sorted(step_results, key=lambda x: -x['accuracy'])[:5]
        print(f"  Top 5 candidatas evaluadas:")
        for j, res in enumerate(top5_candidates):
            marker = " ✓" if res['feature'] == best_feature else ""
            print(f"    {j+1}. {res['feature']}: {res['accuracy']:.4f} ± {res['std']:.4f}{marker}")
    
    return selected_features, history


# Ejecutar SFS
print("\nEjecutando Selección Escalar Hacia Adelante (SFS)...")
...
# Resumen final
print(f"\n\n{'='*70}")
print(f"RESULTADO: 5 MEJORES CARACTERÍSTICAS POR SELECCIÓN HACIA ADELANTE")
print(f"{'='*70}")

print(f"\n{'Paso':<6}{'Característica Añadida':<40}{'Accuracy Acumulado':<20}")
print("-" * 66)
...
for i, feat in enumerate(selected_features, 1):
    fisher_val = fisher_df[fisher_df['Característica'] == feat]['Fisher_Score'].values
    fisher_str = f"{fisher_val[0]:.4f}" if len(fisher_val) > 0 else "N/A"
    print(f"  {i}. {feat}  (Fisher Score: {fisher_str})")

# Evaluación final con las 5 features seleccionadas vs todas
print(f"\n\n{'='*70}")
...
for clf_name, clf in [('KNN (k=5)', KNeighborsClassifier(n_neighbors=5)),
    ...
    print(f"\n{clf_name}:")
    print(f"  Con 5 features seleccionadas: {scores_selected.mean():.4f} ± {scores_selected.std():.4f}")
    print(f"  Con todas ({X_scaled.shape[1]}) features:    {scores_all.mean():.4f} ± {scores_all.std():.4f}")

# Gráfica de la evolución del SFS
...
results_summary = {
    ...
    'sfs_history': [{
        'step': h['step'],
        'feature_added': h['feature_added'],
        'accuracy': h['accuracy']
    } for h in sfs_history],
...
print(f"  - fig7_sfs_results.png         (resultados SFS)")
